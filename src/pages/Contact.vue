<template>
  <div class="narrow">
    <h1>Contact</h1>
    <p>
      For bugs, feature requests, or other feedback please email
      <a href="mailto:feedback@debatewithdata.org"
        >feedback@debatewithdata.org</a
      >
      or
      <a
        href="https://github.com/maxbogue/debatewithdata/issues"
        target="_blank"
        rel="noopener"
        >file an issue</a
      >
      on GitHub.
    </p>
    <p>
      Follow us
      <a
        href="https://twitter.com/debatewithdata"
        target="_blank"
        rel="noopener"
        >@debatewithdata</a
      >
      on Twitter for updates about the project.
    </p>
    <p>
      For all other inquiries please email
      <a href="mailto:contact@debatewithdata.org">contact@debatewithdata.org</a
      >.
    </p>
  </div>
</template>

<script>
export default {
  metaInfo: {
    title: 'Contact',
  },
};
</script>
